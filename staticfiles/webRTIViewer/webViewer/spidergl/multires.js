...
function createRtiViewer(idDiv, imageUrl, width, height)
{
	...
	divHelp.innerHTML = '<div id = "guideTable"><div id = "guideCell"> <div id = "guideList"><h3>WebRTIViewer<br/></h3><ul><li>Pan: LeftMouseButton + MouseMove.</li><li>Zoom in: MouseWhell or press the button <span><img src = "/static/webRTIViewer/webViewer/css/icons/zoomin.png" alt=""/></span></li><li>Zoom out: MouseWheel or press the button <span><img src = "/static/webRTIViewer/webViewer/css/icons/zoomout.png" alt=""/></span></li><li>To change the light direction: activate the light with the button <span><img src = "/static/webRTIViewer/webViewer/css/icons/light.png" alt=""/></span> and press LeftMouseButton + MouseMove or Ctrl + LeftMouseButton + MouseMove.</li><li>Fullscreen: press the button <span><img src = "/static/webRTIViewer/webViewer/css/icons/full.png" alt=""/></span> to active the fullscreen mode and the button <span><img src = "/static/webRTIViewer/webViewer/css/icons/full_on.png" alt=""/></span> to exit</li><li>To reset the viewpoint: press R.</li></ul><a href="http://vcg.isti.cnr.it/rti/webviewer.php" target = "_black">Visual Computing Lab ISTI CNR Pisa</a></div></div></div>'
	divHelp.style.display = "none";
	canvasNode.append(divHelp);
	
	function resetHoverState(btnType)
	{
		touchEvent = false;
		$( "#"+idDiv + "_div button" ).removeClass("ui-state-hover ui-state-active ui-state-focus");
	}
	...
	$( "#"+idDiv + "_div #zoomIn" ).button({
	  ...
	  text: false,
      label: "Zoom In"
    }).on("pointerdown", function(e){multiResRTI.setRepeat(0); if (e.originalEvent.pointerType == "touch") touchEvent = true; else touchEvent = false;})
	.on("pointerup", function(e){multiResRTI.clearRepeat(e); resetHoverState(0);})
	.on("pointerleave", function(e){multiResRTI.clearRepeat(e); resetHoverState(0);})
	.click(function(){resetHoverState(0);});
	...
	$( "#"+idDiv + "_div #zoomOut" ).button({
	  ...
	  text: false,
      label: "Zoom Out"
    }).on("pointerdown", function(e){multiResRTI.setRepeat(1); if (e.originalEvent.pointerType == "touch") touchEvent = true; else touchEvent = false;})
	.on("pointerup", function(e){multiResRTI.clearRepeat(e); resetHoverState(1);})
	.on("pointerleave", function(e){multiResRTI.clearRepeat(e); resetHoverState(1);})
	.click(function(){resetHoverState(1);});
	...
	.on("pointerup", function(e){resetHoverState(2);})
	.on("pointerleave", function(e){resetHoverState(2);})
	.click(function(){resetHoverState(2);});
	...
	$( "#"+idDiv + "_div #help" ).button({
      ...
      label: "Help"
		...
		if (e.originalEvent.pointerType == "touch")
			...
			touchEvent = false;
		$( "#"+idDiv + "_div a" ).unbind("click");
		$( "#"+idDiv + "_div a" ).click(function (e) {return false;});
		$( "#"+idDiv + "_guide" ).show();
		setTimeout(function(){
					$( "#"+idDiv + "_div a" ).unbind("click");
					$( "#"+idDiv + "_div a" ).click(function(e){e.stopPropagation();});
					resetHoverState(4);
					}, 300);
	});
	...
	function fullScreenOn(){
		...
		fullscreen = true;
		var buttonSize = 60;
		var buttonSize2 = (screen.height - 120) / 5;
		if (buttonSize2 < buttonSize)
			buttonSize = buttonSize2;
		$( "#"+idDiv + "_div button" ).css("height", buttonSize);
		$( "#"+idDiv + "_div button" ).css("width", buttonSize);
		...
		setTimeout(function(){resetHoverState(4);}, 300);
	};
	...
	function fullScreenOff(){
		...
		//$( "#"+idDiv + "_div .ui-button" ).removeClass("toolbarButton2");
		//$( "#"+idDiv + "_div .ui-button" ).addClass("toolbarButton");
	...
	$( "#"+idDiv + "_guide" ).on("click", function(){ 
											if (touchEvent)
												return;
											$( "#"+idDiv + "_guide" ).hide();
											resetHoverState(4);});
	...
	{
		divError.innerHTML = '';
		multiResRTI = new MultiRes(canvasName);
...
MultiRes.prototype = {
	...
	repeatEvent: function (type)
	{
		switch (type)
		{
			case 0: this.startZoomIn(); break;
			case 1: this.startZoomOut(); break;
		}
	},
	...
	startPinch: function(gl, delta, x, y)
	{
		this.endAnimation = false;
		this.currentPoint = [x, y];
		this.animationStack = [];
	...
	pinch: function(gl, delta, x, y)
	{
		var  s = delta;
		var temp = this.mat[0] * delta;
		if (temp  > this.maxScale)
			s = this.maxScale / this.mat[0];
		if (temp < 1.0 )
			s = 1.0 / this.mat[0];
		...
		var delta = this._adjustTranslation(tx - this.currentPoint[0], ty - this.currentPoint[1]);
		var deltaX = delta[0];
		var deltaY = delta[1];
		...
		this.currentPoint[0] = tx;
		this.currentPoint[1] = ty;
		
		var tempVect = [this.translation[0], this.translation[1]];
		var tmpMat = sglMulM4(sglTranslationM4C(centerX-tempVect[0], centerY-tempVect[1], 0.0), sglMulM4(sglScalingM4C(s, s, 1.0), sglMulM4(sglTranslationM4C(tempVect[0]-centerX, tempVect[1]-centerY, 0.0), this.mat)));
		var m = this.returnModelMatrix(this.translation, tmpMat);
		var lb = sglMulM4V4(m, sglV4C(this.leftBottom[0], this.leftBottom[1], 0.0, 1.0)).slice(0,2);
		var rt = sglMulM4V4(m, sglV4C(this.rightTop[0], this.rightTop[1], 0.0, 1.0)).slice(0,2);
		lb[0] -= this.viewerdx;
		...
		{
			centerY = this.ui.height / 2.0 - this.translation[1];
			if (rt[0] < this.ui.width)
				deltaX += this.ui.width - rt[0];
			else if (lb[0] > 0)
				deltaX -= lb[0];
		...
		{
			centerX = this.ui.width / 2.0 - this.translation[0];
			if (rt[1] < this.ui.height)
				deltaY += this.ui.height - rt[1];
			else if (lb[1] > 0)
				deltaY -= lb[1];
		...
		{
			centerX = this.ui.width / 2.0;
			centerY = this.ui.height / 2.0;
		}
		this.mat = sglMulM4(sglTranslationM4C(centerX-tempVect[0], centerY-tempVect[1], 0.0), sglMulM4(sglScalingM4C(s, s, 1.0), sglMulM4(sglTranslationM4C(tempVect[0] - centerX, tempVect[1] - centerY, 0.0), this.mat)));
...
function MultiResRenderer(gl, cacheSizeInBytes, onLoad)
 {
	...
	// box rendering
	/******************************************************/
	var bvs = sglLoadFile("/static/webRTIViewer/webViewer/spidergl/box.v.glsl");
	var bfs = sglLoadFile("/static/webRTIViewer/webViewer/spidergl/box.f.glsl");
...
MultiResRenderer.prototype = {
	...
	loadImage: function(url) {
		...
		let pk = currentUrl.split('/').filter(Boolean).pop();  // Extract the pk from the URL
		
		// Fetch the name using pk
		fetch(`get_item_name/${pk}/`)
		  .then(response => response.json())
		  .then(data => {
			let itemName = data.name; // Store the fetched name
	  
			// Proceed with the XMLHttpRequest using the itemName
			xhttp.open("GET", `/media/${itemName}/info.xml`, false);
			xhttp.send();
	  
			var doc = xhttp.responseXML;
			this.format = "jpg";
			var val = parseInt(doc.getElementsByTagName("MultiRes")[0].getAttribute("format"));
			if (!isNaN(val) && val == 1) this.format = "png";
			...
			// Handle shaders as before based on the enumType
			switch (this.enumType[this.type]) {
			  case 1:
				var vs = sglLoadFile("/static/webRTIViewer/webViewer/spidergl/multi.v.glsl");
				var fs = sglLoadFile("/static/webRTIViewer/webViewer/spidergl/hsh.f.glsl");
			  ...
			  case 2:
				var vs = sglLoadFile("/static/webRTIViewer/webViewer/spidergl/multi.v.glsl");
				var fs = sglLoadFile("/static/webRTIViewer/webViewer/spidergl/lrgbptm.f.glsl");
				var prg = new SglProgram(this.gl, [vs], [fs]);
				log(prg.log);
				this._program = prg;
				break;
			  case 3:
				var vs = sglLoadFile("/static/webRTIViewer/webViewer/spidergl/multi.v.glsl");
				var fs = sglLoadFile("/static/webRTIViewer/webViewer/spidergl/rgbptm.f.glsl");
				var prg = new SglProgram(this.gl, [vs], [fs]);
				log(prg.log);
				this._program = prg;
				break;
			  case 4:
				var vs = sglLoadFile("/static/webRTIViewer/webViewer/spidergl/multi.v.glsl");
				var fs = sglLoadFile("/static/webRTIViewer/webViewer/spidergl/image.f.glsl");
				var prg = new SglProgram(this.gl, [vs], [fs]);
				log(prg.log);
				this._program = prg;
				break;
